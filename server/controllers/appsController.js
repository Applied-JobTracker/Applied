...
const appsController = {
  ...
  editApp: async (req, res, next) => {
    ...
    const query = `UPDATE ${tableName} SET company_name = '${company_name}', date = '${date}', app_form = '${app_form}', stack = '${stack}', progess='${progress}' WHERE application_id = ${application_id} RETURNING *`;
    try {
      const result = await db.query(query);
